...
CREATE OR REPLACE FUNCTION location.search_locations_primary_text(candidates text[], area text) RETURNS TABLE("like" types.location_name_and_search_query_with_id)
    LANGUAGE plpgsql
    AS $$
        ...
        FOREACH candidate IN ARRAY candidates
            LOOP
                BEGIN
					...
					WHERE main_text_searcheable_index_col @@ to_tsquery(candidate) AND secondary_text_searcheable_index_col @@ to_tsquery(area);
                EXCEPTION
                    WHEN OTHERS THEN
