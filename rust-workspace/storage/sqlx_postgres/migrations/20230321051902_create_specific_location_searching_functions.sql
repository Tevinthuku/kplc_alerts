...
CREATE OR REPLACE FUNCTION location.search_specific_location_primary_text(candidates text[], location_id uuid) RETURNS TABLE("like" types.location_name_and_search_query_with_id)
    LANGUAGE plpgsql
    AS $$
        ...
        FOREACH candidate IN ARRAY candidates
            LOOP
                BEGIN
					...
					WHERE main_text_searcheable_index_col @@ to_tsquery(candidate) AND id = location_id;
                EXCEPTION
                    WHEN OTHERS THEN
